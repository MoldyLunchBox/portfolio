...
loader.load('Rubik.json', function (font: Font) {
    ...
    const ThreeDTextComponent = () => {
      ...
      const lightTarget1 = new THREE.Vector3(0, 1, 0);
    return (
        <>



            <PerspectiveCamera makeDefault position={[0, 2, 0]} lookAt={() => lookAtTarget} />
            <OrbitControls autoRotate ref={orbitControlsRef} maxPolarAngle={angleToradians(80)} minPolarAngle={angleToradians(60)} />
            
            <mesh ref={ballRef} position={[-1.2, 0, -5]} castShadow >
            ...
            <spotLight   penumbra={0.3} args={["white", 2, 20, 0.3]} castShadow  target={ballRef.current ? ballRef.current : undefined} position={[-5, 5, 2]} />
            {/* environement */}
